...
SECRET_KEY = os.environ.get('SECRET_KEY', 'django-insecure-navdu-cms-secret-key-2026')
...
INSTALLED_APPS = [
    ...
    # Local apps - Unified in Central Super Admin
...
# Multi-Database Configuration for Unified Super Admin Panel
def parse_db_url(url_str, default_name='cms_db'):
...
APPS_DB_URL = os.environ.get('APPS_DB_URL', DATABASE_URL.replace('/cms_db', '/apps_db') if DATABASE_URL else None)
EVENTS_DB_URL = os.environ.get('EVENTS_DB_URL', DATABASE_URL.replace('/cms_db', '/events_db') if DATABASE_URL else None)

DATABASES = {
    'default': parse_db_url(DATABASE_URL, 'cms_db'),
    'apps_db': parse_db_url(APPS_DB_URL, 'apps_db'),
