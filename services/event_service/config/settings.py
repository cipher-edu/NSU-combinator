...
SECRET_KEY = os.environ.get('SECRET_KEY', 'django-insecure-navdu-events-secret-key-2026')
...
def parse_db_url(url_str, default_name='events_db'):
...
CMS_DB_URL = os.environ.get('CMS_DB_URL', DATABASE_URL.replace('/events_db', '/cms_db') if DATABASE_URL else None)
APPS_DB_URL = os.environ.get('APPS_DB_URL', DATABASE_URL.replace('/events_db', '/apps_db') if DATABASE_URL else None)

DATABASES = {
    'default': parse_db_url(DATABASE_URL, 'events_db'),
